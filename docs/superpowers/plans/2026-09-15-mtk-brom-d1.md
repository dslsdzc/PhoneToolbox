...
```cpp
    ...
    // （Task 2 实施时另加的判别力补强用例由实现者命名并声明 —— 见其测试文件；此处只列计划要求的槽）
    void versionFilterSkipsTooNewEntryPlacedFirst();
...
**诚实边界小节（必须写进文档）**：真机全链未验证；0xD8 老格式、PMTv1/v3、MMM-EMI 分支无真实样本；内置芯片表覆盖度 = 转写时的 mtkclient 版本；EMI 缺 preloader 时跳过 DRAM 初始化（DA2 可能起不来）；网络获取默认关闭且**不内置任何第三方来源**。
