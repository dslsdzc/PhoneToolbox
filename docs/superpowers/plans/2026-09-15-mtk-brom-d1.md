...
**Interfaces:**
- Consumes: 既有 `echoCmd` / `readStatus` / `IBromUsb`（`tests/test_mtk_brom.cpp:8-44` 的 `MockUsbChannel`）
- Produces:
  ```cpp
...
void TestMtkBrom::getHwCodeSplits32BitReply()
{
    ...
    // echo 0xFD，回 4 字节；再回 2 字节状态（<= 0xFF 成功）
    m->reads << QByteArray("\xFD", 1) << QByteArray("\x67\x65\xCA\x00", 4) << QByteArray("\x00\x00", 2);
    ...
    QCOMPARE(m->writes.first(), QByteArray("\xFD", 1));
}
...
void TestMtkBrom::getHwSwVerParsesBigEndianQuad()
{
    ...
    QCOMPARE(m->writes.first(), QByteArray("\xFC", 1));
}
```
...
namespace {
// 读满 n 字节；不足即失败（带中文上下文）。dst 可为 nullptr（只读走不解析）。
bool readExactBytes(IBromUsb *u, int n, const QString &what, QByteArray *dst, QString *error)
{
    QByteArray b;
    if (!u->read(b, n, 3000, error) || b.size() != n) {
        if (error && error->isEmpty())
            *error = QStringLiteral("%1 读取不足（要 %2 字节，得 %3）").arg(what).arg(n).arg(b.size());
        return false;
    }
    if (dst) *dst = b;
    return true;
