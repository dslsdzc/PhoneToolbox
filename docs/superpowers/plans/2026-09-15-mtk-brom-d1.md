...
**诚实边界小节（必须写进文档）**：真机全链未验证；**mock 只看写、看不见"读了多少字节"**（`MockUsbChannel::read` 是 `takeFirst+left(maxLen)`：**欠读会无声通过** —— T6 审查实测，读侧判别力只能靠"逐条对上游读长度"+ probe，见 `mtk_payload.cpp` 的 `readExactBytes`/`readExact`）；0xD8 老格式、PMTv1/v3、MMM-EMI 分支无真实样本；内置芯片表覆盖度 = 转写时的 mtkclient 版本；EMI 缺 preloader 时跳过 DRAM 初始化（DA2 可能起不来）；网络获取默认关闭且**不内置任何第三方来源**；**IoT 芯片与"选中条目 DA 空 region"的机型在 D1 刷不了（明确报错，不静默）** —— Task 2 复审实测：2625 的 `0x2523`（上游选中 `region[1].m_len == 0` 的条目 = 0 字节 DA1）、iot 的 `0x6261`（`region[0]/[1]` 都空）在本层会因"零长度 region 显式跳过（P7）"走到"无可用候选"而报错；这类 hw_code 需要上游那套 0 字节/特殊映射语义，**D1 明确不做**（报错优于静默刷坏）。
