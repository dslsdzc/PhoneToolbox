...
12. **P4 0xD8/0xDC 探测必须与 `0x6C + count*size <= filesize` 交叉校验**，不自洽就报错（探测点落在 entry[0] 自己的槽内，`nregions ≥ 10` 时会变成真实数据）。
...
- **DA 条目选择规则**（`daconfig.py:208-218`）：按 `dacode`（默认 = hw_code）过滤 → 再过滤 `hw_version <= 设备hwVer` 且 `sw_version <= 设备swVer`（**设备值为 0 时旁路**）→ 取最兼容。
...
void TestMtkDaFile::probesOldFormatWithCrossValidation()
{
    ...
    QVERIFY2(mtkbrom::parseDaFile(buildDa({entryWith3Regions()}, false, /*oldFormat=*/true), f, &err),
             qPrintable(err));
    QVERIFY(f.oldFormat);
    ...
    for (const QJsonValue &fv : report) {
        ...
        for (int i = 0; i < entries.size(); ++i) {
            ...
            for (int r = 0; r < regions.size(); ++r) {
                ...
                QCOMPARE(int(e.regions.at(r).startAddr), ro.value("m_start_addr").toInt());
...
bool parseDaFile(const QByteArray &data, DaFile &out, QString *error)
{
    ...
    out.isV6 = data.left(kHeaderSize).contains(QByteArray("MTK_DA_v6", 11));
...
4. 取最兼容：`hwVersion` 最大者，再 `swVersion` 最大者；仍并列 → 取**最前**一个 + `warnings` 记明（P2 的 5 元组才是唯一键，版本过滤后仍可能并列）。
    ...
    for node in ast.walk(tree):
        ...
        for key, value in zip(node.value.keys, node.value.values):
            ...
            entries.append((hw, dacode if dacode is not None else hw,
                            damode if damode is not None else 3))
    entries.sort()
    return entries
...
{
    return {QByteArray("\x5A", 1), be32v(16), QByteArray("\x00\x01", 2), be32v(0),
...
{
    return {QByteArray("\xD7", 1), be32v(sel.da1.startAddr), be32v(sel.da1.len), be32v(sel.da1.sigLen),
            QByteArray("\x00\x00", 2), QByteArray("\x00\x00\x00\x00", 4),
            QByteArray("\xD5", 1), be32v(sel.da1.startAddr), QByteArray("\x00\x00", 2)};
}
} // namespace
...
void TestMtkPayload::bringUpOrdersEmiAfterStorageInfoAndBeforeDa2()
{
    ...
    for (int i = 0; i < m->writeFrames.size(); ++i) {
        ...
        if (idxDa1Payload >= 0 && idxBootHeader < 0 && m->writeFrames.at(i) == be32v(sel.da2.startAddr))
...
void TestMtkPayload::bringUpAbortsBeforeDa2WhenSyncIsWrong()
{
    ...
    for (const QByteArray &f : m->writeFrames)
        QVERIFY2(f != be32v(sel.da2.startAddr), "DA1 未就绪时不得进入 boot_to(DA2)");
}
