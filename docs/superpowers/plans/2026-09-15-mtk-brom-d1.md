...
- **LEGACY 没有 `SYNC` / `SETUP_ENVIRONMENT` / `SETUP_HW_INIT_PARAMS`** —— 这三条只存在于 XFlash/XML。DA1 起来后**先回单字节 `0xC0`**（`dalegacy_lib.py:596-601`（实测））。
...
- **EMI 提取算法**（`daconfig.py:120-164`）：搜 `MMM\x01\x38\0\0\0` → `mlen@+0x20` / `siglen@+0x2C` → `data[:mlen-siglen]` → `dramsize = <u32 @ data[-4:]`（**小端**，`DC:127` 的 `unpack("<I", …)`；为 0 则先 `data[:-0x800]` 再读）→ `data[-dramsize-4:-4]` → 搜 `MTK_BLOADER_INFO_v`（无 → 失败）→ **LEGACY 分支**：`data[find("MTK_BIN")+0xC:]`。
...
- **DA 条目选择规则**（`daconfig.py:207-218`）：按 **`chipconfig.dacode`** 作为查找键（**不是** 0xFD 的 hwCode —— 89 条里 31 条 dacode≠hwCode，LEGACY 5 条；T9 修复轮实测）过滤 → 再过滤 `hw_version <= 设备hwVer` 且 `sw_version <= 设备swVer`（设备值为 0 时旁路，上游 `or self.config.hwver == 0`）→ **取文件顺序上首个满足者**（`if self.da_loader is None:` = first-match，不是"取最大版本"；Task 2 审查 Important-1 已核实）。同处 `daconfig.py:200` 的 `if da.hw_code != 0:` 表明**上游在装载阶段就把 `hw_code==0` 占位条目剔掉**（Task 2 审查 Important-2）。
...
- **QtTest 宏里不要内联含 `//`（URL）或括号的 raw string**（`R"(...)"`）—— Qt 6.11.2 的 moc 会误词法化，
  报 `missing ')' in macro usage`；JSON 等一律先落变量再进宏（T8 实施期实测踩到并绕过）。
...
2. **镜像大于分区 = 跳过不写**（Task 7）：写超分区会覆盖相邻分区数据（不可逆、可砖）。
   **落地更正（T12 终审 M2）**：初版写的"用户要强写走既有单分区入口"**在 BROM 模式下不成立** ——
   `flash_panel.cpp:359-369` 对 `MODE_MTK_BROM` 清空分区表并 early-return，`mtkWritePartition` 对 BROM
   **永不可达**。故本期的如实口径 = **超分区镜像只能跳过，无强写出口**（见事实报告 §9）。
...
`test_mtk_payload` 的用例经 `mtktest::makeSelection()` 调用 `parseDaFile`/`selectDaEntry`（`mtk_da_file.cpp`）
→ 必须补进该目标的 `_test_extra_sources`，否则链接期炸（undef）：
...
（`mtk_preloader_emi.h` 是头文件（`EmiData` 只作参数类型）→ 本目标**不需要**它的 .cpp；T9 才引入。）
...
bool BromSession::getHwCode(quint16 &hwCode, quint16 &hwVer, QString *error)
{
    ...
    if (!echoCmd(CMD_GET_HW_CODE, error))
        return false;
    QByteArray b;
    if (!m_usb->readExact(b, 4, 1000, error))   // 传输层保证恰好 4 字节（累加见 IBromUsb::readExact）
        return false;
...
bool BromSession::getHwSwVer(HwSwVer &out, QString *error)
{
    ...
    if (!echoCmd(CMD_GET_HW_SW_VER, error))
        return false;
    QByteArray b;
    if (!m_usb->readExact(b, 8, 1000, error))   // 恰好 8 字节
        return false;
...
- Produces:
  ...
  // 失败（无 MTK_BLOADER_INFO_v / 无 MTK_BIN / 切片为空 / 版本字节非数字）→ false + 中文 error；
//    ↑ 与上游净效果一致：上游在 `int()` 抛异常时走 `except: emiver = 0; emi = None`（daconfig.py:**162-164**），
//      即 EMI 不可用、根本不发 ENABLE_DRAM（**不是** 157-160 那段 `self.error(...)/exit(1)` 硬中止）
...
//   [0, 18)  = "MTK_BLOADER_INFO_v" + "38\0\0"（版本 = 标记后 2 个 ASCII 字节；"MTK_BLOADER_INFO_v" 是 **18** 字符）
...
static QByteArray buildPreloader(int emiLen)
{
    ...
    d.replace(0, 18, QByteArray("MTK_BLOADER_INFO_v", 18));   // 18 字节（写 16 = 丢掉 "_v"，夹具里就没有标记了）
    d.replace(18, 2, "38");   // ⚠️ 版本字节写在**标记之后**（标记占 [0,18) —— 写成 16 会把 "_v" 覆盖掉）
...
void TestMtkPreloaderEmi::realPreloaderExtracts()
{
    ...
    QVERIFY(!pre.isEmpty());
    // 该样本的真实特征（样本侦察实测）：`MTK_BLOADER_INFO_v35` @254392、MTK_BIN @254492
    QCOMPARE(pre.indexOf(QByteArray("MTK_BLOADER_INFO_v")), 254392);
    QCOMPARE(pre.indexOf(QByteArray("MTK_BIN")), 254492);
    QCOMPARE(pre.mid(254392 + 18, 2), QByteArray("35"));   // 版本字节就是 "35" → 35（不是末位 5）

    mtkbrom::EmiData emi;
    QString err;
    QVERIFY2(mtkbrom::extractEmiLegacy(pre, emi, &err), qPrintable(err));
    QCOMPARE(emi.ver, quint32(35));   // 判据 = 标记后 **2 个 ASCII 字节**整体（"35" → 35；不得只取末位）
...
bool extractEmiLegacy(const QByteArray &preloader, EmiData &out, QString *error)
{
    ...
    bool ok = false;
    // 版本 = 标记后 2 字节，**去 NUL 后必须全是数字**；否则**整体提取失败**（false + error）。
    // ⚠️ 不得"非数字 → ver=0 且提取成功"：上游在 `int()` 抛异常时是 `except: emiver = 0; emi = None`
    // （daconfig.py:157-160），即 **EMI 不可用、根本不会发 ENABLE_DRAM**；而 ver=0 在上游是**合法档位**
    // （`emiver == 0` 走 tier-0：写 0xFFFFFFFF + 截断 EMI）。把"读不懂"混进"合法的 0"会让伪造版本驱动协议分档。
    // 合规的 "00" 仍然接受（int("00") == 0，上游同）。
    // **只去尾部 NUL**（上游 `.rstrip(b"\x00")` 语义）—— 去全部 NUL 会比上游**放宽**：
    // 上游 `int(b"\x005")` 抛异常 → emi = None（不发 EMI），而"去掉内嵌 NUL 得 '5'"会照发 tier-5。
    QByteArray digits = verBytes;
    while (digits.endsWith('\0'))
        digits.chop(1);
    bool ok = digits.size() == 1 || digits.size() == 2;   // 去尾部 NUL 后 1-2 位数字
    for (const QChar c : digits)
        if (!c.isDigit()) ok = false;
    if (!ok) {
        if (error) *error = QStringLiteral("EMI 版本字节不是数字（读到 %1）—— 与上游一致按**提取失败**处理"
                                           "（上游此时 emi = None，不发 DRAM 配置）")
                                .arg(QString::fromLatin1(verBytes));
        return false;
    }
    out.ver = quint32(QString::fromLatin1(digits).toUInt());
...
- Modify: `src/core/modes/mtk_handler.cpp`（**过渡**：`runBromFlash` 里那 3 行 `sendPayload` 调用改成**明确失败**
  「需先解析 DA 文件并走 DA1/DA2 两阶段（Task 9）」+ `Q_UNUSED(daBinary)`；枚举/连接/target_config 骨架保留，
  头文件注释不动 —— T9 会重写整个 `runBromFlash`。**不这么改主程序与 test_pipeline 都编不过**）
...
void TestMtkPayload::sendEmiLegacyZeroVersionFollowsUpstreamSequence()
{
    ...
    QCOMPARE(m->writeFrames.at(6), QByteArray("\x80\x00\x00\x01", 4));     // >I 0x80000001（BE = 80 00 00 01）
...
void TestMtkPayload::readFlashInfoDa2ConsumesAllBytesAndChecksPassInfo()
{
    ...
    nand[11] = char(0x00); nand[12] = char(0x00);                    // NandInfo32 count @11 = 0 → **上游读 0 字节**
...
// 上游 `mtk_config.py:231-283 bmtsettings(hwcode)` 的 **eMMC 分支全表**（D1 只走 eMMC；nand 分支不实现，
// 由 sendStage2Config 的 `flashtype != "emmc" → 明确报错` 挡住）。逐条核对（T6 实施期）：
//   {0x6592, 0x8127, 0x6571} emmc → flag 1 / 0x1500000（:235-239；0x8127 同时出现在 :240 的列表里，
//        但 235 分支在前 → **先命中者胜**）
//   {0x6570,0x8167,0x6580,0x6735,0x6753,0x6755,0x6752,0x6595,0x6795,0x6767,0x6797,0x8163} → flag 1 / 0（:240-243，**不看 flashtype**）
//   {0x6575} emmc → flag 1 / 0x1500000（:253-260）
//   {0x6582} emmc → **flag 2** / 0x1500000（:261-265）
//   {0x6572} emmc → **flag 0 / 0xA8**（:266-273）—— 上游这里疑似把自己的 blockcount 写进了 partsize
//        （nand 分支才是 0x1500000），但**那是真机收到的字节** → 忠实复刻 + 注释，不"替它修正"
//   {0x6577,0x6583,0x6589} emmc → 走默认（:274-278 只处理 nand）
//   其余 hwcode → 默认 flag 1 / 0
BmtSettings bmtSettings(quint16 hwCode)
{
    ...
    else if (hwCode == 0x6575)
        s.partSize = 0x1500000;
    else if (hwCode == 0x6582) {                     // **flag 2** 且 partSize 0x1500000（两处都要）
        s.flag = 2;
        s.partSize = 0x1500000;
    } else if (hwCode == 0x6572) {                   // 上游怪异值：flag 0 + partSize 0xA8（照抄，勿"修正"）
        s.flag = 0;
        s.partSize = 0xA8;
    }
    return s;
                      ...
                      const QString &flashtype, bool *emiNeeded, QStringList *log, QString *error)
{
    ...
    if (hwCode == 0x6592) {
        ...
        if (!wr(be32(1)))                                        // slc_percent = 1
            return false;
        if (!wr(QByteArray("\x46\x46\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xFF\x00\x00\x00", 20)))   // **20 字节**（dalegacy_lib.py:264 实测；声明 19 会截掉末尾 0x00）
            return false;
...
// ⚠️ nandcount 两级都为 0 时上游是 `usbread(-4)` —— **读 0 字节**（不是"读到没有为止"！）：
//    `Connection/usblib.py:462-467` 对 `resplen <= 0` 只 `self.info("Warning !")`，随后 `bytestoread = resplen`
//    使 `while bytestoread > 0` **直接不成立** → 返回空（`:526-537` 的 `return res[:resplen]`）。
//    ⚠️ 这是 **eMMC 手机的常态路径**（无 NAND → nandcount == 0）：若在此"有界读到没有为止"，
//    会把随后的 info2/EMMC/SDC/flashconfig/PassInfo 段**一并吃掉** → 后续全错位（T6 审查 S1 Critical）。
bool readFlashInfoDa2(BromSession &s, quint16 hwCode, QStringList *log, QString *error)
{
    ...
    if (nandcount == 0) {
        ...
        if (nandcount == 0) {
            // 上游 `usbread(-4)` = 读 0 字节 —— 这里**什么都不读**（切勿"有界读"，会吃掉后续段）
            while (total < 0x1000) {
                QByteArray chunk;
            ...
            if (log)
                *log << QStringLiteral("read_flash_info：NAND 计数为 0（上游 usbread(-4) 读 0 字节）")
                            .arg(total);
        } else if (nandcount > 2 &&
...
```bash
git add src/core/modes/mtk_brom.h src/core/modes/mtk_brom.cpp src/core/modes/mtk_payload.h src/core/modes/mtk_payload.cpp tests/test_mtk_payload.cpp tests/test_mtk_brom.cpp tests/mtk_test_helpers.h CMakeLists.txt
...
- Produces:
  ...
  // MTK 固件包内的 Android_scatter.txt（只取 partition_name / partition_size；两种写法都认：
//   带 '-' 的 `- partition_index` 与缩进的裸键 `partition_name:`）；0 个分区 → 明确失败。
...
2. **镜像大于分区 → 跳过**（不是照写）：写超分区会覆盖相邻分区数据（不可逆、可砖）。判据只认"分区大小已知且镜像更大 → 跳过 + 逐条告警"。~~用户要强写走既有单分区入口~~ → **落地更正（T12 终审 M2）**：BROM 模式下分区表被清空、单分区入口不可达，**无强写出口**；`mtk_flash_plan` 不提供覆盖开关（见事实报告 §9）。
...
bool parseScatter(const QString &text, QList<PartitionRef> &out, QString *error)
{
    ...
    for (const QString &raw : lines) {
        ...
        // ⚠️ **只有 `partition_index` 带 '-'**，`partition_name`/`partition_size` 是**缩进裸键**
        //    （实测真样本：`- partition_index` 50 条、`- partition_name` **0** 条）→ 必须剥可选前导 '-'，
        //    否则一行都解析不出来（T7 实施期发现的 brief 自相矛盾）。
...
- Produces:
  ...
  // 语义（spec §7）：**false ⟺ 显式路径失败**（T8 实施期把不变量收敛成这一句）——
  // 显式给了路径却读不到 = 用户以为会做 DRAM 初始化 → 必须响亮失败；
  // 自动导入读不到 / 网络不可用 / 缓存写失败 → 一律只进 skipReason + log，**不中止刷写**。
...
// ⚠️ 这两个**可测**（T8 审查实测）：`qputenv("XDG_CONFIG_HOME", tmpDir.path())` 后
//    `configuredSourcesPath()` 落到临时目录，三个分支（缺失 / 合法 / 非法清单）+ log 文案全部可在
//    QTemporaryDir 内覆盖，**零 $HOME 写入**。实现者初版以"要写 $HOME"为由跳过了覆盖 —— 那是错的。
                      ...
                      const PreloaderDownloader &downloader, PreloaderResult &out, QString *error)
{
    ...
    if (candidates.size() == 1) {
        // ⚠️ 唯一候选读不到/为空 → **不中止**（T8 实施期更正：本函数的契约是
        //    **false ⟺ 显式路径失败**；自动导入/网络/缓存全是"尽力而为"，失败只进 skipReason + log）
        QString whyNot;
        if (!readWholeFile(candidates.first(), out.bytes, &whyNot)) {
            out.log << QStringLiteral("自动导入的 preloader 不可用：%1").arg(whyNot);
            out.skipReason = QStringLiteral("固件目录里的唯一 preloader 候选读不到/为空（%1）—— 跳过 DRAM 初始化")
    ...
    for (const PreloaderSource &src : sources) {
        ...
        // 缓存写不进去**不影响使用**（字节已通过 sha256 校验；缓存只是便利）→ 只告警、path 留空
        QString cachePath;
        QString cacheErr;
        if (!writeCache(opt.cacheDir, src.name, data, &cachePath, &cacheErr))
            out.log << QStringLiteral("preloader 缓存写入失败（不影响本次使用）：%1").arg(cacheErr);
...
void TestMtkPayload::bringUpOrdersEmiAfterStorageInfoAndBeforeDa2()
{
    ...
    // ⚠️ pre.bytes 必须是**合法 preloader**（含 "MTK_BLOADER_INFO_v" + 版本字节 + "MTK_BIN"），
    //    否则 extractEmiLegacy 会失败、bring-up 走失败路径（T9 实施期发现的 brief 夹具错）。
    //    版本写 "00" → ver == 0（合法 tier-0），与 emiReadsVer0() 的档位一致。
    //    ⚠️ 该夹具**要新建**：把 `buildEmiPreloader(verBytes, emi)` 加到 `tests/mtk_test_helpers.h`
    //       （标记 18 字节 + verBytes + MTK_BIN + 尾部 EMI；`tests/test_mtk_preloader_emi.cpp:41` 已有一个
    //       **文件内 static** 的同类（`buildPreloader(int)`）—— 不动它，两者统一留给终审 triage）。
    pre.bytes = mtktest::buildEmiPreloader(QByteArray("00"), QByteArray(16, '\xA5'));
                  ...
                  const BromProgressFn &progress, QString *error)
{
    ...
    mtkbrom::DaSelection sel;
    QStringList selWarn;
    // ⚠️ **查找键是 chip->dacode，不是 0xFD 的 hwCode**（T9 修复轮实测上游 `daconfig.py:208-209`：
    //    `dasetup` 按**条目自己的 hw_code** 建表（`:190/:192/:200`），`setup()` 用 `chipconfig.dacode` 查。
    //    本仓 89 条里有 31 条 dacode != hwCode，其中 LEGACY 代 5 条（0x0321/0x0335/0x0337→0x6735、
    //    0x0507→0x6758、0x8172/0x8176→0x8173）—— 传 hwCode 会让这些机型在"无候选"处响亮失败（不砖机但刷不了）。）
    if (!mtkbrom::selectDaEntry(daFile, chip->dacode, sw.hwVer, sw.swVer, &selWarn, sel, error)) {
...
（`mtk_handler.cpp` 需新增 include：`core/mtk_flash_plan.h`、`core/modes/mtk_da_file.h`、`core/modes/mtk_payload.h`、`core/modes/mtk_preloader_emi.h`、`core/modes/mtk_preloader_fetch.h`、`<QFile>`、`<utility>`；**旧 `runBromFlash(daBinary, partitions, error)` 签名保留为薄弃用桩** —— 它的调用方 `flash_tool.cpp:1247`
   同时被**主程序**与 **test_pipeline** 编译，删净会让 T9 提交到 T10 落地之间构建中断（T6 踩过同一个行序错误）。
   桩体只写明确失败：「旧入口已废弃：flash_tool 通道接线属 Task 10（runBromFlash(BromFlashRequest)）」，
   并注明「T10 接线时删除」；**T10 必须删掉它**（那是 T10 的收尾项之一）。）
...
- ⚠️ **验收项：删掉 T6/T9 留下的旧 `runBromFlash(daBinary, partitions, error)` 薄弃用桩**（它的调用方 `flash_tool.cpp:1247` 在本任务里改成新签名）—— 桩体注释里已写明"T10 接线时删除"
- Produces:
  ```cpp
...
- [ ] **Step 4: CMake —— 核对（**T9 已补齐，勿重复追加**）**

T9 为了让"每个提交都可构建"已经给 `test_pipeline` 的 `_test_extra_sources` 加了 D1 的依赖源
（`mtk_flash_plan.cpp` / `mtk_da_file.cpp` / `mtk_chip_table.cpp` / `mtk_preloader_emi.cpp` /
`mtk_preloader_fetch.cpp` / `mtk_preloader_download_qt.cpp`）—— 本步只需**确认它们在**，
**不要重复追加**（重复会出现同名源两次，Ninja 报重复规则）。
...
> **T10 审查交接的两项（本任务一并解决，别留给终审）**：
> 1. **入口噪声**（`flash_tool.cpp:1273-1276`）：`loadConfiguredSources` **无条件**落日志 —— 不需要 preloader 的用户每次刷写都会看到"…网络获取不可用"这种误导性噪音。改法：**只在 `allowNetwork` 为真时**才加载来源清单并落日志（关网时那份清单根本用不上，连文件都不用读）。
> 2. **进度条不重绘**：整包路径（`flash_panel.cpp:746-762`）没有 `processEvents` 泵，而 EDL 计划路径有（`:1268-1271`）→ `progressFn` 会 emit，但进度条要等阻塞调用返回才刷新。**照 EDL 那条既有模式**补上（或明确写成"进度在大块写完后跳变"的已知限制 —— 但优先照既有模式）。
> 另：T10 把通道改成"缺镜像 = 响亮失败"后，`flash_panel.cpp:352` / `:703` / `:757` 三处旧文案（"分区镜像选择待接线"/"本次仅执行 DA 协议握手"）**先承诺后失败**，必须随本任务改掉。
...
| `read_flash_info` 里 **两级 nandcount == 0 时 `usbread(-4)` 读 0 字节**（不是"读到超时"——**eMMC 手机常态路径**） | `Connection/usblib.py:462-467`（`resplen<=0` 只 info 一句，`while bytestoread>0` 不成立）+ `:526-537` 的 `return res[:resplen]`；调用点 `dalegacy_lib.py:526-551` |
| `bmtflag/bmtpartsize` 的 **eMMC 全表**（含 `0x6575` 0x1500000、`0x6582` flag2+0x1500000、**`0x6572` flag0 + 0xA8** 的上游怪异值） | `mtk_config.py:231-283` |
...
**诚实边界小节（必须写进文档）**：真机全链未验证；**枚举/打开/握手、代际拒绝、0xFC 降级、空计划早拒、FINISH 只告警这些真机段只有代码级保证、无 mock 用例**（T9 自报，报告 §7）—— 交付文档不得把它们写成"已验证"；**构建的"0 warning"只是编译器默认档**（`CMakeLists.txt` 未开 `-Wall/-Wextra` —— T7 审查指出；想要更强的静态保证需另行开启，那会引入一批既有告警，属独立工程）；**mock 只看写、看不见"读了多少字节"**（`MockUsbChannel::read` 是 `takeFirst+left(maxLen)`：**欠读会无声通过** —— T6 审查实测，读侧判别力只能靠"逐条对上游读长度"+ probe，见 `mtk_payload.cpp` 的 `readExactBytes`/`readExact`）；0xD8 老格式、PMTv1/v3、MMM-EMI 分支无真实样本；内置芯片表覆盖度 = 转写时的 mtkclient 版本；EMI 缺 preloader 时跳过 DRAM 初始化（DA2 可能起不来）；网络获取默认关闭且**不内置任何第三方来源**；**IoT 芯片与"选中条目 DA 空 region"的机型在 D1 刷不了（明确报错，不静默）** —— Task 2 复审实测：2625 的 `0x2523`（上游选中 `region[1].m_len == 0` 的条目 = 0 字节 DA1）、iot 的 `0x6261`（`region[0]/[1]` 都空）在本层会因"零长度 region 显式跳过（P7）"走到"无可用候选"而报错；这类 hw_code 需要上游那套 0 字节/特殊映射语义，**D1 明确不做**（报错优于静默刷坏）。
