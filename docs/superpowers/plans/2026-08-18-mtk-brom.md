...
```
3. 在 foreach 循环结束后（`endif()` 之前，即 `endforeach()` 之后）追加 libusb 链接与 include（image_engine 不链 libusb，测试目标需显式加）：
```cmake
...
{
    // 对照 sdmmc_switch_part()：发 0x60 → ACK → 发 parttype → ACK
    if (!m_session.echoCmd(DA_CMD_SDMMC_SWITCH_PART, error))
        return false;
    QByteArray ack;
                         ...
                         int partType, QString *error)
{
    ...
    if (!switchPart(partType, error))
        return false;
    if (!m_session.echoCmd(DA_CMD_READ, error))
        return false;
    QByteArray hdr;
...
bool DaStorage::listPartitions(QList<EmPartition> &out, QString *error)
{
    ...
    for (int pos = 0; pos + entrySize <= pd.size(); pos += entrySize) {
        EmPartition p;
        p.name = QString::fromUtf8(pd.mid(pos, 0x40)).trimmed();
        if (entrySize == 0x60) {
...
- Produces:
  - `bool mtkbrom::sendPayload(BromSession &s, const QByteArray &daBinary, QString *error)`：SEND_DA（地址/长度/签名长度从 DA 载荷头部解析或默认 0x0 地址）+ JUMP_DA。**诚实边界**：DA 二进制需调用方提供（从官方固件/设备提取的自研工具为后续任务）；V6 新平台 BROM 修补（jump_da 后 DA 同步失败）返回明确错误
...
void TestMtkPayload::flashPartitionWritesAtPartitionOffset()
{
    ...
    pd[0x48] = char(0xFF);
    memcpy(pd.data(), "boot", 4);
    memcpy(pd.data() + 0x40, "\x00\x00\x00\x00\x00\x10\x00\x00", 8); // size
    memcpy(pd.data() + 0x50, "\x00\x00\x00\x00\x00\x10\x00\x00", 8); // offset
    m->reads << QByteArray(1, char(0x5A))
...
// SEND_DA 上传（地址默认 0，签名长度默认 0）→ JUMP_DA。DA 二进制需调用方提供。
bool sendPayload(BromSession &s, const QByteArray &daBinary,
                 quint32 address = 0, quint32 sigLen = 0, QString *error = nullptr);
...
bool sendPayload(BromSession &s, const QByteArray &daBinary, quint32 address,
                 quint32 sigLen, QString *error)
{
    // 对照 upload_da1()：send_da(addr, size, sig_len, da) → jump_da(addr)
    if (daBinary.isEmpty()) {
        if (error) *error = QStringLiteral("DA 二进制为空（需调用方提供，自研提取为后续任务）");
        return false;
    }
    if (!s.sendDa(address, quint32(daBinary.size()), sigLen, daBinary, error))
        return false;
    if (!s.jumpDa(address, error))
        return false;
    return true;
}
...
```
3. libusb 链接条件追加 `OR _test_stem STREQUAL "test_mtk_payload"`。
                  ...
                  QString *error)
{
    ...
    if (!mtkbrom::sendPayload(session, daBinary, 0, 0, error))
        return false;
