...
{
    // 规格 §3.5：发 0x60 → ACK → 发 parttype → ACK（DA 命令无回显，ACK 即应答）
...
- **F1 最终审查（final review）**：F1-2 草图同步 —— switchPart/emmcRead 的 DA 命令从 echoCmd 改 raw write（DA 命令无回显、ACK 即应答，与 merged mtk_emmc.cpp 一致）；listPartitions 名字解析同步为首个 NUL 截断（QString::trimmed 不剥离 NUL）；另 F1-1/F1-2 注释统一为规格文档章节引用（洁净室纪律，mtk_brom.h 顶部保留一处 mtkclient 子模块核实注）、握手加 3 次重试 + 失败后清残留输入（规格 §2.1 重试语义）、V6 措辞修正（无专用检测，以 SEND_DA/JUMP_DA 失败传播）。
