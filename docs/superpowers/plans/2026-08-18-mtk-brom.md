...
**SEND_DA**（`send_da()` + `upload_data()`）：echo 0xD7 → echo addr(4B BE) → echo len(4B BE) → echo sig_len(4B BE) → 读 status(2B)；0x1D0D→SLA 错误；≤0xFF → 分块（wMaxPacketSize）写数据，每 0x2000 字节补空包，收尾补空包 → 读 4B（checksum 4B BE + status 2B BE，小端无，均为大端读取，resp = checksum(4B) + status(2B) 实为 rword(2) 两次：先 checksum 后 status——按 `rword(2)` 返回 tuple (checksum, status)）；checksum≠0 且≠本地异或和 → 警告不终止；status≤0xFF 成功。
