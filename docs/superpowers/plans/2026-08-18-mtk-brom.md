...
**EMMC 写**（`dalegacy_lib.py sdmmc_write_data()`）：发 0x62 → storage(1B: 2=EMMC) → parttype(1B) → addr(8B BE) → length(8B BE) → packetsize(4B BE, 0x100000) → 读 1B==ACK；循环：发 ACK → 数据块(≤0x100000，512 对齐补 0) → checksum(2B BE, 逐字节和 & 0xFFFF) → 读 1B==CONT(0x69)。
...
bool DaStorage::listPartitions(QList<EmPartition> &out, QString *error)
{
    ...
    const int entrySize = marker == 0xFF ? 0x60
                          : (quint64(quint8(pd.size() > 0x4B ? pd[0x48] : 0)) == 0
                             ? 0x58 : 0x4C); // 见下方修正
    // 注：0x58/0x4C 判定对照 read_pmt()：mask_flags = <Q partdata[0x48:0x4C]，
    // 0 < mask_flags < 0xA → 0x58 条目；否则 0x4C。此处简化：marker!=0xFF 时按
    // partdata[0x48] 小端 u32 判定。
...
**注意**：`listPartitions` 的 0x58/0x4C 判定处有简化，实现时以测试为准（测试只覆盖 0x60 路径，0x58/0x4C 路径按对照源码实现 `mask_flags = quint64(quint8(pd[0x48])) | ... ` 完整小端 u64 判定：`quint64 maskFlags = getLe64(pd, 0x48); entrySize = (maskFlags > 0 && maskFlags < 0xA) ? 0x58 : 0x4C;` —— 以此为准替换上述简化表达式）。
