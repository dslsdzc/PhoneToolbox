...
**D10 — 交接不做自动跳转。** 完成后提示"设备应已进入 Download 模式，请继续刷写"。
~~并在检测到 Download 模式设备时把提示点亮~~ —— **该后半句本期未实现（T8 收尾时定案，spec → 计划 掉了这个钩子）**：
对话框是无 DeviceDetector 引用的静态模态流程，接口里没有承载"检测到 Download 设备"的位置。**事实上的指示器**是
设备重枚举为 Odin 后 FlashPanel 的模式显示与按钮回到「刷入」（`flash_panel.cpp:381-392`），用户据此即可确认交接成功。
若日后要"点亮提示"，入口是给对话框加一个 DeviceDetector 信号订阅（或在 FlashPanel 侧把重枚举事件转成对话框的一次刷新）。
