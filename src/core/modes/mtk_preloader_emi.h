...
// XFlash 走 INIT_EXT_RAM + 4B 长度帧，见 mtk_xflash_payload）。
//
...
struct EmiData {
    QByteArray bytes;    // 待发送的 EMI 本体 —— 取法**由调用的是哪个函数决定**：
                         // extractEmiLegacy → MTK_BIN+0xC 起；extractEmiXflash → 整个 dramsize 窗口
...
// 纯函数：从 preloader 字节提取 XFlash 用的 EMI —— **整个 dramsize 窗口**（上游 daconfig.py:137-139
// 的 `idx == 0 且 damode == XFLASH` 分支整块返回；本实现把 `damode` 的选择留给调用方，见 .cpp）。
