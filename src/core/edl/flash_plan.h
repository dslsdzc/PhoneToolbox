...
// **未声明单位时是 warn-only**（元数据的 `SECTOR_SIZE_IN_BYTES` 缺失或不可用 —— 为 0 / 不可解析
// 时同样落到这一支，`haveSectorSize == false`；而 GPT 的 LBA 尺寸不等于条目将采用的默认值 4096）→ 对账照常（值按现状，不换算、不丢弃），**另加一条中文告警**说明
