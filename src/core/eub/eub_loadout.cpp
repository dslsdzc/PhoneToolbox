...
//
// ⚠️ 本线**无真机**（sboot.bin 是三星签名二进制，本仓不**分发**它，facts §F1/§F8）；真样本已就位
// （reference/eub-samples/，facts §H）并由 gated 用例实跑核对（含与表无关的结构地标）：
// 下表只到"照抄参照实现 + 与 spec 逐值对齐 + 真样本核对"这一层证据，**未在设备上验证过**。布局表绑定
