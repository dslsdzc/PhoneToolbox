...
// ============================================================
// 最小 ZIP 读取（PKWARE 规范；与 magisk_patcher.cpp 内实现同源 ——
// C4 按任务文件清单保持 kernelsu_patcher 自包含，C7 宜合并为共享工具）
// ============================================================
                     ...
                     QString *err)
{
    ...
    if (found->method == 8) {
        // raw DEFLATE（ZIP 条目无 zlib 封装头，inflateInit2(-MAX_WBITS)）
...
// ZIP 条目名列表（供 LKM 包 / ksuinit.zip 按名搜索）。
...
// ============================================================
// newc cpio（070701/070702）读写 —— 与 magisk_patcher.cpp 的 CpioArchive
// 同源（布局与 Magisk cpio 实现逐字节一致），增补 exists/rename/remove
// ============================================================
...
public:
    bool parse(const QByteArray &buf, QString *err)
    {
        ...
        m_entries.clear();
        if (buf.size() < 110)
            return fail("cpio: 数据过短（非 cpio 镜像）");
        int pos = 0;
        while (pos + 110 <= buf.size()) {
            ...
            m_entries.insert(QString::fromUtf8(name), e);
            p += filesize;
    ...
    bool exists(const QString &name) const { return m_entries.contains(name); }

    // 改名（ksud cpio.mv 语义：原样保留模式与内容）
    ...
    // image"）。C3 注入产物含 .backup/.magisk 标记；官方 Magisk 修补产物
    // 亦含 .backup 链 —— 以 .backup 条目存在与否判定，与 C3 防护一致。
